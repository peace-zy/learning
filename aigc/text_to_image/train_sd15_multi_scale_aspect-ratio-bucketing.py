...
#
#     def __iter__(self):
#         # 在每个epoch开始时重置dataloader_iters
#         self.dataloader_iters = list(enumerate([iter(dl) for dl in self.dataloaders]))
#         return self
#
#     def __next__(self):
#         while len(self.dataloader_iters) > 0:
#             # 随机选择一个dataloader
#             # i = random.randint(0, len(self.dataloader_iters) - 1)
#             try:
#                 index, dataloader_iter = self.dataloader_iters[0]
#                 # 尝试从选中的dataloader中获取数据
#                 data = next(dataloader_iter)
#                 # 返回数据和dataloader的标识
#                 return data, index
#             except StopIteration:
#                 # 如果选中的dataloader已经没有数据，那么将其从列表中移除
#                 del self.dataloader_iters[0]
#         # 如果所有的dataloader都没有数据，那么抛出StopIteration异常
#         raise StopIteration
#
#     def __len__(self):
...
class ImageStore:
    def __init__(self, data_csv: str) -> None:
        ...
        self.image_files = ['/aistudio/workspace/aigc_ssd/' + i for i in self.data_df['img_path'].to_list()]
...
class AspectBucket:
    ...
    def get_batch_count(self):
        print([len(b) // self.batch_size for b in self.bucket_data.values()])
...
def main():
    ...
    print(bucket.get_bucket_info(),bucket.get_batch_count())
    ...
    if accelerator.is_main_process:
        ...
        print(args.tracker_project_name, tracker_config)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                # ######################## webui版本 #######################
                prompt_embeds, _ = concat_encode_prompt(tokenizer, text_encoder, device=latents.device,
                ...
                add_time_ids = compute_time_ids(batch['pixel_values'].shape[2:])
