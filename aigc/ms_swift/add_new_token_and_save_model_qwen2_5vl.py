...
from transformers import Qwen2_5_VLForConditionalGeneration, AutoProcessor, AddedToken
...
def add_new_tokens_to_save(model_name_or_path='open_models/Qwen/Qwen2.5-VL-3B-Instruct', output_dir='./new'):
    ...
    print(f'before add new tokens: {len(tokenizer)}')
    new_token_list = []
    max_floor_num = 15 + 1
    ...
    print(f'after add new tokens: {len(tokenizer)}')
    return len(tokenizer)

def init_tokenizer(model_name_or_path='open_models/Qwen/Qwen2.5-VL-3B-Instruct'):
    max_seq_length = 14400
    processor = AutoProcessor.from_pretrained(
        model_name_or_path)
    tokenizer = processor.tokenizer
...
def main():
    ...
    test_jsonl = 'train.jsonl'
    original = 'open_models/Qwen/Qwen2.5-VL-3B-Instruct'
    new = 'open_models/Qwen/Qwen2.5-VL-3B-Instruct-add-token-new'
    ...
    print(f'original total_vocab_size={len(original_tokenizer)}')
    new_tokenizer = init_tokenizer(new)
    print(f'new total_vocab_size={len(new_tokenizer)}')
