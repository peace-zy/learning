...
def train_step(
    ...
    for data in pbar:
        ...
        loss = distillation_loss_fn(student_zh_preds,  teacher_preds)
        losses.append(loss.item())
        #losses.append(loss.sum().item())
...
def train(epoches, teacher, student):
    ...
    optimizer = torch.optim.Adam(student.parameters(), lr=1e-4, weight_decay=0.0001)
...
def main():
    teacher_model = 'bra_v5_en'
    teacher = TeacherModel(teacher_model)
    student_model = 'bra_v5_en'
