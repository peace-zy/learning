...
def get_dinov2_config(model_name, image_classifier=False):
    ...
    config = Dinov2Config(image_size=224, patch_size=14)
...
def convert_dinov2_checkpoint(model_name, pytorch_dump_folder_path, push_to_hub=False):
    ...
    offline_pos_embed = None
    # load original model from torch hub
    if os.path.exists(model_name):
        offline_model = torch.load(model_name)
        model_name = "_".join(model_name.split("_")[:2])
        print(model_name)
        original_model = torch.hub.load("facebookresearch/dinov2", model_name, img_size=224, pretrained=False)
        offline_pos_embed = offline_model["pos_embed"]
        #offline_model["pos_embed"] = original_model.state_dict()["pos_embed"]
        original_model.load_state_dict(offline_model)
        
        #original_model = torch.hub.load(model_name)
    else:
    ...
    if offline_pos_embed is not None:
        state_dict["pos_embed"] = offline_pos_embed
    ...
    with torch.no_grad():
        ...
        original_outputs = original_model(pixel_values.cuda()).cpu()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        choices=[
            "dinov2_vits14",
            "dinov2_vitb14",
            "dinov2_vitb14_offline_train.pt",
            "dinov2_vitl14",
            "dinov2_vitg14",
