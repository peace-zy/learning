...
def parse_args():
    ...
    parser.add_argument("--wrong_file", type=str, help="wrong result files")
                ...
                for idx, ocr_text_item in enumerate(details):
                    if 'auxiliary' in ocr_text_item:
                        ...
                        for graph in paragraphs:
                            ...
                            #cv2.polylines(image, [points], True, (0, 0, 0))
                        ret = ori['ocr']['ret']
                        for r in ret:
        ...
        for (var i = 1; i < show_table.rows.length; ++i) {
            ...
            for (var j = 1; j < cells.length; ++j) {
                ...
                for (var k = 0; k < radios.length; ++k) {
                    ...
                    if (radios[k].checked) {
                        if (radios[k].value == 'Right') {
                            right_line += img_src + "\\t" + text[1] + "\\n"
                        }
                        else if (radios[k].value == 'Wrong') {
                            wrong_line += img_src + "\\t" + text[1] + "\\n"
                        }
                    }
                }
            }
...
'''
    ...
    for info in show_info:
        ...
        for idx, r in enumerate(regions):
            ...
            if gen_region_url in wrongs:
                select = """<label class="radio-inline" style="font-family: 'Microsoft YaHei UI';font-size: medium;">
                            <input type="radio" display:block name="result{}" id="optionsRadios{}" value="Right" onclick="update(this)" />对</label>
                            <label class="radio-inline" style="font-family: 'Microsoft YaHei UI';font-size: medium;">
                           <input type="radio" display:block name="result{}" id="optionsRadios{}" value="Wrong" onclick="update(this)" checked="" />错</label>
                            <label class="radio-inline" style="font-family: 'Microsoft YaHei UI';font-size: medium;">
                           <input type="radio" display:block name="result{}" id="optionsRadios{}" value="Hold" onclick="update(this)" />待定</label>""".\
                           format(id_r, c, id_r, c + 1, id_r, c + 2)
                wrongs.remove(gen_region_url)
...
def main():
    ...
    args = parse_args()
    txt2html(args)
    #python txt2html_for_ocrlib_add_wrong.py --infile /home/work/cloud/project/baidu/adx/ocrlib/part-00973.shuf.1000.ocr --pre_url http://10.12.186.173:8050/for_analysis --save_path ~/cloud/share/for_analysis_add_wrong --show_image_path show --region_path regions --ori_image_path ori --wrong_file wrong.txt
    return
