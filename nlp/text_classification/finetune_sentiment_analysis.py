                    ...
                    if step % 100 == 0:
                        ...
                        with P.no_grad():
                            ...
                            for step, d in enumerate(
                                ...
                                acc.append(a.numpy())
                                pred.append(logits.argmax(-1).numpy())
                                gt.append(label.numpy())
                            model.train()
                        pred = np.concatenate(pred)
                        gt = np.concatenate(gt)
                        log.debug("{}\t{}\t{}\t{}\t{}".format(pred, gt, len(pred), np.sum(pred), np.sum(gt)))
                        ...
                        log.debug('acc %.5f, recall %.5f precision %.5f f1_score %.5f' % (acc, recall, precision, f1_score))
